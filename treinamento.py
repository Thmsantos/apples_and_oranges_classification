...
def preparar_dados(file_path):
    ...
    joblib.dump(scaler, os.path.join(TRAINING_DIR, 'scaler.pkl'))

    label_encoder = LabelEncoder()
    y = label_encoder.fit_transform(y)
    joblib.dump(label_encoder, os.path.join(TRAINING_DIR, 'label_encoder.pkl'))
...
def treinar_modelo(X, y):
    ...
    checkpoint = ModelCheckpoint(os.path.join(TRAINING_DIR, 'model.keras'), save_best_only=True, monitor='val_accuracy', mode='max')
...
def carregar_modelo():
    model = tf.keras.models.load_model(os.path.join(TRAINING_DIR, 'model.keras'))
    scaler = joblib.load(os.path.join(TRAINING_DIR, 'scaler.pkl'))
    label_encoder = joblib.load(os.path.join(TRAINING_DIR, 'label_encoder.pkl'))
...
if __name__ == '__main__':
    file_path = './data/apples_and_oranges.csv'
